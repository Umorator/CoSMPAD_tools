...
import xgboost as xgb  # use XGBoost native load_model
import joblib
...
MODELS_DIR = os.path.join(BASE_DIR, "models")  # point to JSON folder

MODEL_PATHS = [
    os.path.join(MODELS_DIR, "model_fold_0.json"),
    os.path.join(MODELS_DIR, "model_fold_1.json"),
    os.path.join(MODELS_DIR, "model_fold_2.json")
]
...
class CosmpadPredictor:
    def __init__(self):
        ...
        self.feature_order = pd.read_pickle(FEATURE_ORDER_PATH)
    ...
    def predict_from_sequence(self, sequences: list[str]) -> pd.DataFrame:
        ...
        for seq in sequences:
            ...
            model_probas = np.array([model.predict_proba(X)[0] for model in self.models])
            ...
            pred_label_name = self.sp_name_mapping.get(pred_label_short, pred_label_short)

            mean_max_proba = np.mean(np.max(model_probas, axis=1))
            vote_agreement = np.mean(np.argmax(model_probas, axis=1) == pred_index)
